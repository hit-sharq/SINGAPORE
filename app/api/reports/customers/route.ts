...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error instanceof Error && error.message === 'FORBIDDEN') {
      return errorResponse(ErrorCodes.FORBIDDEN, 'You do not have permission to view customer insights', 403, undefined, getPath(request))
    }
    console.error('GET /api/reports/customers error:', error)
    return errorResponse(ErrorCodes.INTERNAL_ERROR, 'Unable to load customer insights', 500, undefined, getPath(request))
  }
}